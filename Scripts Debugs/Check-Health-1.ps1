...
ConvertTo-HTML -Head $Style -PostContent "$ReportHead $OSHead $OS $DiskHead $Disk $AppLogHead $AppEvent $SysLogHead $SysEvent $ServHead $Service $InstalledAppsHead $InstalledApps $HotFixHead $HotFix" -Title "System Health Check Report"  |  Out-File "C:\HealthCheck\$computer\Health Report $CurrentDate.html"